...
function AddEntryModal({
  ...
  const [step, setStep] = useState(1); // 1=subject, 2=details, 3=score
  const [form, setForm] = useState<FormState>({
    subjectCode: '', component: '', year: profile.yearsRange.from,
  ...
  const years = subj
    ? Array.from({ length: subj.yearsRange.to - subj.yearsRange.from + 1 }, (_, i) => subj.yearsRange.from + i)
    : [];
  ...
  const canStep3 = !!form.score && !isNaN(parseFloat(form.score)) && parseFloat(form.score) >= 0;
  ...
  const handleSave = () => {
    ...
    const existing = progress.find(p =>
      p.subjectCode === form.subjectCode && p.component === form.component &&
      p.year === form.year && p.session === form.session
    );

    const newEntries = existing ? progress.filter(p => p.id !== existing.id) : progress;
      ...
      <div className="glass-bright animate-scale-in" style={{
            ...
            {STEPS.map((s, i) => (
              <div key={s} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5 }}>
                <div style={{
                  ...
                  background: i < step ? `linear-gradient(90deg, ${color}, var(--accent-cyan))` : 'rgba(255,255,255,0.08)',
              ...
              {profile.subjects.map(s => {
                ...
                return (
                  <button key={s.code} onClick={() => setForm(f => ({ ...f, subjectCode: s.code, component: '' }))}
                    style={{
                      padding: '14px 16px', borderRadius: 12, cursor: 'pointer', border: 'none', textAlign: 'left',
                      background: sel ? `${c}18` : 'rgba(255,255,255,0.03)',
                      outline: sel ? `2px solid ${c}` : '1px solid var(--border-dim)',
        ...
        {!saved && step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fade-up 0.3s both' }}>
            {/* Component */}
            <div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>Component</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {subjData?.components.filter(c => subj?.components.includes(c.code)).map(c => {
                  const sel = form.component === c.code;
                  return (
                    <button key={c.code} onClick={() => setForm(f => ({ ...f, component: c.code }))}
                      style={{
                        padding: '11px 14px', borderRadius: 10, cursor: 'pointer', border: 'none', textAlign: 'left',
                        background: sel ? `${color}18` : 'rgba(255,255,255,0.03)',
                        outline: sel ? `2px solid ${color}` : '1px solid var(--border-dim)',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                ...
                <select value={form.session} onChange={e => setForm(f => ({ ...f, session: e.target.value as any }))} className="input-field">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: 'fade-up 0.3s both' }}>
            ...
            {canStep3 && (
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Live Preview</div>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', fontWeight: 800, color: gradeColor(grade) }}>{grade}</div>
                </div>
                ...
                <div style={{ position: 'absolute', left: 0, bottom: 0, right: 0, height: 3, borderRadius: '0 0 14px 14px', background: `linear-gradient(90deg, ${color}, var(--accent-cyan))`, width: `${pct}%`, transition: 'width 0.3s' }} />
              </div>
            )}
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {(Object.entries(STATUS_CONFIG) as [Status, typeof STATUS_CONFIG[Status]][]).map(([val, cfg]) => (
                  <button key={val} onClick={() => setForm(f => ({ ...f, status: val }))}
                    style={{
                      padding: '8px 13px', borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none',
                      background: form.status === val ? cfg.bg : 'rgba(255,255,255,0.04)',
                      color: form.status === val ? cfg.color : 'var(--text-muted)',
                      outline: form.status === val ? `2px solid ${cfg.color}60` : '1px solid var(--border-dim)',
                      transition: 'all 0.15s',
                    }}>
                    {cfg.icon} {cfg.label}
                  </button>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {DIFFICULTY_CONFIG.map(d => (
                  <button key={d.value} onClick={() => setForm(f => ({ ...f, difficulty: f.difficulty === d.value ? undefined : d.value }))}
                    title={d.label}
                    style={{
                      flex: 1, padding: '10px 4px', borderRadius: 10, fontSize: 18, cursor: 'pointer', border: 'none',
                      background: form.difficulty === d.value ? `${d.color}20` : 'rgba(255,255,255,0.04)',
                      outline: form.difficulty === d.value ? `2px solid ${d.color}70` : '1px solid var(--border-dim)',
                      transition: 'all 0.15s',
                    }}>
                    {d.emoji}
                  </button>
        ...
        {saved && (
          <div style={{ textAlign: 'center', padding: '20px 0', animation: 'scale-in 0.3s both' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              {grade !== 'U' ? `You scored ${grade} (${pct.toFixed(1)}%) — great work!` : `Saved. Keep practising and you'll improve!`}
            </div>
...
export function ProgressRecorder({ profile, progress, onAddProgress, onUpdateProgress }: Props) {
  ...
  const handleAdd = (entry: ProgressEntry) => {
    ...
    const filtered = progress.filter(p =>
      !(p.subjectCode === entry.subjectCode && p.component === entry.component &&
        p.year === entry.year && p.session === entry.session)
    );
    onUpdateProgress([...filtered, entry]);
  };
  ...
  // Group by subject for summary cards
  const bySubject = profile.subjects.map(s => {
    ...
    const avgPct = done.length > 0
      ? (done.reduce((acc, p) => acc + p.score, 0) / done.reduce((acc, p) => acc + p.maxScore, 0)) * 100
      : 0;
  ...
  return (
    <div style={{ padding: '32px 28px', display: 'flex', flexDirection: 'column', gap: 28, maxWidth: 1100 }}>
          ...
          {bySubject.map((s, i) => (
            <div key={s.code}
              style={{
                ...
                background: filterSubject === s.code ? `${s.color}18` : 'rgba(255,255,255,0.03)',
                border: `1px solid ${filterSubject === s.code ? s.color + '50' : 'var(--border-dim)'}`,
                transition: 'all 0.2s',
      ...
      {filtered.length > 0 ? (
          ...
          {filtered.map((entry, i) => {
            ...
            return (
              <div key={entry.id} style={{
                ...
                animation: `fade-up 0.35s ${Math.min(i * 0.03, 0.25)}s both`,
              }}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{entry.year} · {SESSION_LABELS[entry.session]}</span>
        ...
        /* ── GRID VIEW ── */
        <div className="animate-fade-up stagger-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
          {filtered.map((entry, i) => {
            const subj = profile.subjects.find(s => s.code === entry.subjectCode);
            const color = getSubjectColor(entry.subjectCode).hex;
            const pct = entry.maxScore > 0 ? (entry.score / entry.maxScore) * 100 : 0;
            ...
            return (
              <div key={entry.id}
                style={{
                  ...
                  animation: `fade-up 0.35s ${Math.min(i * 0.03, 0.3)}s both`,
                  boxShadow: '0 2px 12px rgba(0,0,0,0.3)',
                }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-3px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = `0 8px 28px rgba(0,0,0,0.4), 0 0 0 1px ${color}30`; }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 2px 12px rgba(0,0,0,0.3)'; }}
              >
                {/* Top color bar */}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
                        Paper {entry.component} · {entry.year} · {entry.session.toUpperCase()}
                      </div>
                    </div>
                    <div style={{
                      width: 40, height: 40, borderRadius: 10, flexShrink: 0, marginLeft: 8,
                      background: `${color}18`,
        ...
        <div className="glass animate-fade-up stagger-3" style={{ padding: '60px 24px', textAlign: 'center', borderRadius: 'var(--radius-lg)' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 20 }}>
            {progress.length === 0
              ? 'Start by logging your first past paper above'
              : 'Try clearing the filters to see all your papers'}
          </p>
          {progress.length === 0 && (
